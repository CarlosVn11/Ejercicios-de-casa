...
//*************	 Visual Studio 2017					         ****//
//*************  Ejercicio de casa 10: Carro volador         ***//
//*************  Fecha: 05/11/2018                          **//
//***********************************************************//
...
float angRot = 0.0;
float movKitX = 0.0;
float movKitY = 0.0; //para moverse a traves del eje Y
float movKitZ = 0.0;
float rotKit = 0.0;
float rotKitX = 0.0;
float rotTires = 0.0;
float rotLlanta = 0.0; //para rotar llantas 
float rotLlanta1 = 0.0; //para rotar llantas
bool g_fanimacion = false;
bool g_avanza = false;

bool circuito = false;
bool recorrido1 = true; //estado A
bool recorrido2 = false; //ESTADO B
bool recorrido3 = false; //ESTADO C
bool recorrido4 = false; //ESTADO D
bool recorrido5 = false; //ESTADO E
//estados adicionales
bool recorrido6 = false; //ESTADO F
bool recorrido7 = false; //estado intermedio
bool recorrido8 = false; //G
...
void display ( void )   // Creamos la funcion donde se dibuja
{
		...
		glPushMatrix();		
			...
			glPushMatrix();
				...
				glPushMatrix(); //llanta frontal der
					...
					glRotatef(rotLlanta1,0,0,-1); //rotacion de las llantas cuando vuela
				...
				glPushMatrix(); //llanta frontal izq
					glTranslatef(6,-1,7.5);	
					glRotatef(rotLlanta,0,0,1); //rotacion de las llantas cuando vuela
					glRotatef(180,0,1,0);
					glRotatef(rotTires,1,0,0);
					llanta.GLrender(NULL,_SHADED,1.0);
				glPopMatrix();
				...
				glPushMatrix(); //llanta trasera izq
					glTranslatef(6,-1,-9.5);	
					glRotatef(rotLlanta,0,0,1); //rotacion de llanta cuando vuela
					glRotatef(180,0,1,0);
					glRotatef(rotTires,1,0,0);
					llanta.GLrender(NULL,_SHADED,1.0);
				glPopMatrix();
	...
	glDisable(GL_TEXTURE_2D);
		glDisable(GL_LIGHTING);
			...
			pintaTexto(-11,12.0,-14.0,(void *)font,"Ejercicio de casa 10");
...
void animacion()
{
	...
	if(circuito) //bandera de circuito
	{
		...
		if(recorrido6) //estado F
		{
			...
			rotKitX = -45.0;
			movKitZ += 0.1;
			movKitY += 0.33; //variable de control para el eje Y
			rotLlanta -= 0.30;
			rotLlanta1 = rotLlanta - 0.3;
			if (movKitZ >= -125.0)
			{
				recorrido6 = false;
				recorrido7 = true;
		...
		if (recorrido7) { //estado intermedio
			rotKit = 0;
		    rotKitX = 0;
			movKitZ++;
		...
		if (recorrido8) { //estado G
			rotKit = 0.0;
			rotKitX = 45.0;
			movKitZ +=0.1;
			movKitY -= 0.32;
			rotLlanta+= 0.3;
			rotLlanta1 = rotLlanta + 0.30;
			if (movKitZ >= 155) {
				
				movKitY = 0.0; //para completar la distancia que este a 0
				recorrido8 = false;
				recorrido2 = true; //vamos al estado B
			}
		}
